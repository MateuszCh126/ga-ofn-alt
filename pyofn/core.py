...
długości N próbkowane równomiernie dla y z przedziału [0, 1].
...
class OFN:
    ...
    x_up : array-like lub callable
        Ramię wznoszące. Jeśli callable: f(y) -> float/array dla y w [0,1].
    x_down : array-like lub callable
        Ramię opadające. Jeśli callable: f(y) -> float/array dla y w [0,1].
    n : int
    ...
    def up(self) -> np.ndarray:
        """Ramię wznoszące (wartości x dla y w [0,1])."""
        return self._up

    @property
    def down(self) -> np.ndarray:
        """Ramię opadające (wartości x dla y w [0,1])."""
        return self._dn
    ...
    def core(self) -> tuple[float, float]:
        ...
        Dla trójkąta/trapezu oba punkty mogą być równe lub tworzyć przedział.
        """
