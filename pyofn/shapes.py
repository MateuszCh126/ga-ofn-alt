...
def triangular(a: float, b: float, c: float, n: int = 512) -> OFN:
    ...
    Trójkątny OFN skierowany w prawo: wierzchołek w b.
        ...
        a <= b <= c
        x_up  : ramię wznoszące, od a do b   (y od 0 do 1)
        x_down: ramię opadające, od b do c   (y od 1 do 0, parametryzowane przez y od 0 do 1)
...
def triangular_left(a: float, b: float, c: float, n: int = 512) -> OFN:
    ...
    W OFN kierunek w lewo oznacza, że x_up jest MALEJĄCE (od c do b),
    a x_down kontynuuje malejąco (od b do a).
    Interpretacja: OFN "biegnie" z prawej strony w lewo.

        x_up  : od c do b   (y od 0 do 1, wartości x maleją)
        x_down: od b do a   (y od 0 do 1, wartości x maleją)
    """
    if not (a <= b <= c):
        raise ValueError(f"Wymagane a <= b <= c, podano a={a}, b={b}, c={c}")
    y = np.linspace(0.0, 1.0, n)
    x_up   = c - y * (c - b)   # malejące c..b, direction = -1
    x_down = b - y * (b - a)   # malejące: b..a
    return OFN(x_up, x_down, n)
...
def trapezoidal(a: float, b: float, c: float, d: float, n: int = 512) -> OFN:
    ...
    Trapezowy OFN skierowany w prawo: plateau b..c.
        ...
        a <= b <= c <= d
        x_up  : od a do b   (y od 0 do 1)
        x_down: od c do d   (y od 1 do 0, parametryzowane przez y od 0 do 1)
...
def gaussian(mean: float, sigma: float, n: int = 512, support_sigma: float = 3.0) -> OFN:
    ...
    Przy y bliskim 0 wartości rosną bez ograniczenia, więc obcinamy do support_sigma*sigma.
    """
    if sigma <= 0:
    ...
    x_up = mean - half_width          # lewa strona: maleje dla y bliskiego 0
...
def linear_ofn(
    ...
    Parametry (wszystkie dla y z przedziału [0, 1]):
        x_up_start   = x_up(y=0)
        x_up_end     = x_up(y=1)
...
def about(value: float, spread: float = 1.0, n: int = 512) -> OFN:
    """
    Symetryczny trójkąt "około value" plus/minus spread.
