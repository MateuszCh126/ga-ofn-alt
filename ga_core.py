...
Reprezentacja:
  gen       = trapezoid OFN (a, b, c, d), a <= b <= c <= d
  chromosom = tablica (n_genes, 4)
  populacja = tablica (pop_size, n_genes, 4)

Cel (target) budowany jest przez pyofn.trapezoidal, a najlepszy osobnik
może być wyeksportowany do listy obiektów pyofn.OFN.

Fitness to odległość Hamminga między trapezami, liczona analitycznie
na całej populacji jedną operacją numpy. Dla trapezu (a,b,c,d):
    x_up(y)   = a + y*(b-a)
    x_down(y) = d - y*(d-c)
Różnica ramion osobnika i celu jest liniowa w y, więc całkę
z |alpha*y + beta| po y w [0,1] liczymy ze wzoru na pole trapezu/trójkąta.
"""
...
# ---------------------------------------------------------------------------
# Całka z |alpha*y + beta| po y w [0, 1]
# ---------------------------------------------------------------------------

def _integral_abs_linear(alpha: np.ndarray, beta: np.ndarray) -> np.ndarray:
    """
    Całka z |alpha*y + beta| dy po przedziale [0,1] dla tablic alpha, beta
    (dowolny kształt).

    Funkcja jest liniowa, więc pole pod wartością bezwzględną to trapez
    albo dwa trójkąty:
        v0 = beta          (y=0)
        v1 = alpha + beta  (y=1)
      1) v0 i v1 tego samego znaku: pole = (|v0| + |v1|) / 2
      2) zmiana znaku w y0 = v0/(v0-v1): dwa trójkąty
    """
    v0 = beta
    v1 = alpha + beta
...
def fitness_batch(population: np.ndarray, target: np.ndarray) -> np.ndarray:
    ...
    Returns
    -------
    fitness : (P,)  - średnia odległość Hamminga po genach (mniej = lepiej)
    """
...
def individual_to_ofns(individual: np.ndarray, n: int = 256) -> List[OFN]:
    """
    Konwertuje chromosom (n_genes, 4) na listę n_genes obiektów pyofn.OFN.
...
def mutate_gaussian(population: np.ndarray, rate: float, sigma: float,
    ...
    mutated.sort(axis=-1)   # wymusza a <= b <= c <= d
    return mutated
...
class OFNGeneticAlgorithm:
    ...
    Cel budowany jest przez pyofn.trapezoidal, wynik można wyeksportować
    przez best_as_ofns(). Fitness, selekcja, krzyżowanie i mutacja są
    wektoryzowane w numpy.
    """
    ...
    def _build_target(self):
        """Buduje tensor celu (G, 4) na podstawie pyofn.trapezoidal."""
        cfg = self.cfg
    ...
    def update_target(self, a: float, b: float, c: float, d: float):
        """Zmiana celu bez restartu populacji."""
        v = sorted([a, b, c, d])
    ...
    def best_as_ofns(self, n: int = 256) -> List[OFN]:
        ...
        Używane przez app.py do rysowania.
        """
