"""
GUI aplikacji OFN GA Explorer (tkinter + matplotlib).
...
from ga_core import OFNGeneticAlgorithm, GAConfig, GAStats, fitness_batch, ofn_to_params
...
class OFNGAApp:
    ...
    def _setup_window(self):
        self.root.title("OFN GA Explorer - trapezoidalne OFN")
    ...
    def _build_left(self, parent):
        ...
        # TARGET OFN - 4 parametry trapezu (na dole panelu)
        S(); H("TARGET OFN  (trapezoid)").pack(**P); S()
    ...
    def _build_evo_panel(self, parent):
        ...
        self._ax_heat.set_title(
            "Ewolucja populacji (zielony: blisko celu, czerwony: daleko). Kliknij lub przeciągnij.",
    ...
    def _init_mpl_main(self):
        ...
        self._ax_fit.set_title("Historia fitness (odległość Hamminga)",
    ...
    def _draw_best_genes(self, s: GAStats):
        """Rysuje wszystkie geny najlepszego osobnika jako trapezy OFN."""
        ...
        # najlepszy chromosom jako lista pyofn.OFN
    ...
    def _draw_comparison(self, s: GAStats):
        """Porównuje cel z najlepszym genem najlepszego osobnika."""
        ax = self._ax_cmp; ax.cla()
        _ax_defaults(ax)
        ax.set_title("Cel  vs  Najlepszy gen (z najlepszego osobnika)", color=TEXT, fontsize=9, pad=4)
        ...
        # najlepszy gen: ten o najmniejszym dystansie do celu
        if s.best_individual is not None and s.best_individual.size > 0:
            try:
                genes = s.best_individual                       # (G,4)
                # Fitness per-gen (analitycznie, jak w ga_core.fitness_batch)
                pop = genes[:, None, :]                          # (G,1,4)
                tgt_params = ofn_to_params(tgt_ofn)              # (4,)
                tgt = tgt_params[None, :]                        # (1,4)
                dists = fitness_batch(pop, tgt)                  # (G,)
                best_g = int(np.argmin(dists))

                best_params = genes[best_g]
                best_ofn = trapezoidal(*best_params.tolist(), n=200)
                _draw_single_ofn(ax, best_ofn, GREEN, alpha_fill=0.12, lw=1.8, ls_down=":")
                ax.set_xlabel(f"Najlepszy gen index: {best_g}  |  dist: {float(dists[best_g]):.5f}",
                              color=MUTED, fontsize=7)
            except Exception:
                pass
        ...
        ax.legend(handles=[
            ...
            Patch(color=GREEN,  label="Najlepszy gen"),
